...
type ConflictsWithType struct {
	Keys []string
}

func (j ConflictsWithType) Description(ctx context.Context) string {
	return ""
}

func (j ConflictsWithType) MarkdownDescription(ctx context.Context) string {
	return ""

}

func (j ConflictsWithType) Validate(ctx context.Context, request tfsdk.ValidateAttributeRequest, response *tfsdk.ValidateAttributeResponse) {
	...
	for _, v := range j.Keys {
		...
		if len(paths) > 0 {
			...
			if !isNull {
				response.Diagnostics.AddError(
					fmt.Sprintf("Conflict in paths"), // value in field: %v", ),
					fmt.Sprintf("Path '%v' conflict with '%v", request.AttributePath.String(), tftypes.NewAttributePathWithSteps(steps).String()),
				)
				return
			}
		}
...
func ConflictsWith(items ...string) ConflictsWithType {
	var values []string
	...
	return ConflictsWithType{Keys: values}
}
